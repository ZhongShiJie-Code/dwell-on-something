...
import { createFcmSender, classifyFcmError, validateFcmData } from './fcm-sender.mjs';
...
test('task notification routes reach the Firebase sender validation path', async t => {
  const root = await tempRoot();
  t.after(() => fsp.rm(root, { recursive: true, force: true }));
  const setup = await invalidCredentialEnv(root);
  t.after(() => fsp.rm(setup.credentialRoot, { recursive: true, force: true }));
  ...
  const result = await sender.send({
    ...
    data: {
      v: '1',
      notification_epoch: 'epoch-1',
      device_id: 'device-1',
      notification_id: '1',
      kind: 'task',
      title: 'Claude Cli',
      body: '任务已完成',
      route: 'task/task-1/run-1',
      at: '1',
    },
  });

  assert.deepEqual(result, { ok: false, class: 'sender_unavailable', code: 'firebase_admin_unavailable' });
  await sender.close();
});

test('FCM route validation matches the Android task-route contract', () => {
  const base = {
    v: '1',
    notification_epoch: 'epoch-1',
    device_id: 'device-1',
  ...
  assert.equal(validateFcmData({ ...base, route: 'task/task-1/run-1' }).route, 'task/task-1/run-1');
  for (const route of [
    'task/task-1',
    'task/task-1/run-1/extra',
    'task/task%2F1/run-1',
    'task/%ZZ/run-1',
    'task/%2E/run-1',
    'task/task-1/run%20one',
    'task/task-1/run-1?query',
    'task/task-1/run-1#fragment',
    'task/task-1/run-1\\escape',
  ]) {
    assert.throws(
      () => validateFcmData({ ...base, route }),
      error => error?.code === 'invalid_fcm_route',
      route,
    );
  }
});
